...
export function useScriptData(script: any, setScript: (updater: (prev: any) => any) => void) {
   ...
   * 更新分镜数据（局部刷新）
   */
  const updateShotData = useCallback(async (
    shotId: number, 
    data: any, 
    options?: { 
      showMessage?: boolean;
   ...
   * 删除分镜数据（局部刷新）
   */
  const deleteShotData = useCallback(async (shotId: number) => {
    try {
   ...
   * 更新分镜中特定视频的状态（局部刷新）
   */
  const updateVideoStatus = useCallback((shotId: number, videoId: number, updates: any) => {
    setScript((prevScript: any) => {
      if (!prevScript) return prevScript;
   ...
   * 删除分镜中的图片（局部刷新）
   */
  const removeShotImage = useCallback((imageId: number) => {
    setScript((prevScript: any) => {
      if (!prevScript) return prevScript;
   ...
   * 更新图片的首帧/尾帧标记（局部刷新）
   */
  const updateImageFrameStatus = useCallback((shotId: number, imageId: number, frameType: 'first' | 'last') => {
    setScript((prevScript: any) => {
      if (!prevScript) return prevScript;
